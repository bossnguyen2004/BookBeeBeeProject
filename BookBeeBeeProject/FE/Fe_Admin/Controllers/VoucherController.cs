...
namespace Fe_Admin.Controllers
{
    public class VoucherController : Controller
    {
        ...
        public VoucherController(IHttpClientFactory httpClientFactory, ITempDataProvider tempDataProvider)
        {
        ...
        public async Task<IActionResult> Index(int currentPage = 1, int pageSize = 10, string key = "")
        {
            ...
            var url = $"https://localhost:7287/api/Voucher?page={currentPage}&pageSize={pageSize}{(string.IsNullOrEmpty(key) ? "" : $"&key={Uri.EscapeDataString(key)}")}";
            ...
            var vouchers = json?["data"]?.ToObject<List<Voucher>>() ?? new List<Voucher>();
        ...
        public async Task<IActionResult> Create(VoucherDTO model)
        {
            ...
            var url = "https://localhost:7287/api/Voucher";
        ...
        public async Task<IActionResult> Edit(int id)
        {
            ...
            var url = $"https://localhost:7287/api/Voucher/{id}";
            ...
            try
            {
                ...
                var publisher = JsonConvert.DeserializeObject<VoucherDTO>(await response.Content.ReadAsStringAsync());
