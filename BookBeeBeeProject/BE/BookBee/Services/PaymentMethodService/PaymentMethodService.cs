...
namespace BookBee.Services.PaymentMethodService
{
    public class PaymentMethodService : IPaymentMethodService
    {
        ...
        public PaymentMethodService(IPaymentMethodRepository paymentMethodRepository, IMapper mapper, DataContext dataContext)
        {
        ...
        public async Task<ResponseDTO> CreatePaymentMethod(PaymentMethodDTO paymentMethodDTO)
        {
            ...
            var success = _paymentMethodRepository.CreatePayment2(payment);
            if (success.IsSuccess == true) return new ResponseDTO() { Message = "Tạo thành công" };
        ...
        {
            return _dataContext.PaymentMethods.ToList();
        }
        public async Task<List<DetailedPayment>> GetAllDetailedPaymentAsync()
        {
            return await _dataContext.DetailedPayments.ToListAsync();
        }
        public async Task<ResponseDTO> CreateToDetailPayment(DetailedPayment detailedPayment)
        {
            await _dataContext.DetailedPayments.AddAsync(detailedPayment);
            await _dataContext.SaveChangesAsync();
            return new ResponseDTO { IsSuccess = true, Code = 200, Message = "Thành công" };
        }

        public ResponseDTO CreateToDetailPayment2(DetailedPayment detailedPayment)
        {
            try
            {
                _dataContext.DetailedPayments.Add(detailedPayment);
                _dataContext.SaveChanges();
                return new ResponseDTO { Code = 200, Message = " thành công" };
            }
            catch (Exception)
            {
                return new ResponseDTO { IsSuccess = false, Code = 500, Message = "Thất bại" };
            }
